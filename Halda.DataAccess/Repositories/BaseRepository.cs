...
namespace Halda.DataAccess.Repositories
{
    public abstract class BaseRepository<TEntity, TKey> : IRepository<TEntity, TKey>
        where TEntity : class
    {
        protected readonly HaldaDbContext _dbContext;
        private readonly DbSet<TEntity> _dbSet;
        protected int CommandTimeout { get; set; }

        public BaseRepository(HaldaDbContext dbContext)
        {
        ...
        public virtual async Task<IList<TEntity>> GetDynamicAsync(CancellationToken token, Expression<Func<TEntity, bool>> filter = null,
            string orderBy = null,
            Func<IQueryable<TEntity>, IIncludableQueryable<TEntity, object>> include = null,
            bool isTrackingOff = false)
        {
            ...
            if (orderBy != null)
            {
                ...
                if (isTrackingOff)
                    return await result.AsNoTracking().ToListAsync(token);
                else
